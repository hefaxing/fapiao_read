...
def subprocess_popen(CMD):
    ...
    try:
        ...
        while P_CMD.poll() is None:
            time.sleep(1)
            run_now = datetime.datetime.now()
            if (run_now - run_start).seconds > timeout:
                subprocess.Popen('cmd /c taskkill /f /t /im acrobat.exe')
        P_CMD.wait()
    ...
    for file_name in file_names:
        ...
        if is_create_file:
            ...
            if new_file_name in  new_file_sum.keys():
                ...
                new_file_name = '%s.%s' % (new_file_name, str(int(time.time())))
            else:
            ...
            try:
                s = subprocess_popen('wmic process where name="Acrobat.exe" delete')
